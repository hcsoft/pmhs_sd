...
public class InfectionReportService extends HealthMainService<InfectionReportBO> {
    ...
    public Object check(Map data) throws Exception {
        Map ret = new HashMap();

        try {
            TaxempDetail user = cn.net.tongfang.framework.security.SecurityManager.currentOperator();
            int id = Integer.parseInt((String) data.get("ID"));
            int orgid = Integer.parseInt((String) data.get("OrgID"));
            //有主键,进行update
            String cols = "";
            Map updateMap = new HashMap();
            updateMap.put("sCheckPerson", true);
            updateMap.put("dCheckDate", true);
            updateMap.put("iState", true);
            for (Object key : data.keySet()) {
                String keystr = (String) key;
            ...
            cols = cols.substring(1);
            String updatesql = " update  Center_Infectious set  " + cols + " where ID = ? and orgid = ?  ";
            System.out.println("updatesql====");
            System.out.println(updatesql);
            SQLQuery query = getSession().createSQLQuery(updatesql);
            int idx = 0;
            for (Object key : data.keySet()) {
                ...
                if (updateMap.containsKey(key)) {
                    ...
                    System.out.println(" s====" + s);
                    if (s != null && BusiUtils.isDate(s)) {
                        System.out.println(" date idx====" + idx);
                        query.setParameter(idx++, BusiUtils.parseDate(s));
    ...
    public Map newGet(int id,int orgid) throws Exception {
        QueryRunner run = new QueryRunner(SessionFactoryUtils.getDataSource(getSessionFactory()));
        return run.query("select * from Center_Infectious where id = ? and orgid = ? ", new MapListHandler(), id,orgid).get(0);
    }
